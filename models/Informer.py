...
class series_decomp_multi(nn.Module):
    ...
    def forward(self, x):
        ...
        for func in self.moving_avg:
            ...
            moving_mean.append(moving_avg_out)
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        # Learnable linear projections for trend / seasonal components
        ...
        # Feature Mixer
        self.mlp_feat_list = nn.ModuleList([
            Mlp_feat(configs.enc_in, configs.enc_in)
            for _ in range(self.mlp_layers)
        ])
        # Time Mixer
        self.mlp_time_list = nn.ModuleList([
            Mlp_time(self.seq_len, self.seq_len, n_vars=configs.enc_in)
            for _ in range(self.mlp_layers)
        ])
        self.act_list = nn.ModuleList([
            nn.ReLU() for _ in range(self.mlp_layers)
        ])
        self.dropout_list = nn.ModuleList([
            nn.Dropout(0.1) for _ in range(self.mlp_layers)
        ])
        # Linear Fusion
        self.lin_fuse_list = nn.ModuleList([
        ...
        embed_cls = embed_type_map.get(configs.embed_type, DataEmbedding)
        self.enc_embedding = embed_cls(
            configs.enc_in, configs.d_model, configs.embed, configs.freq, configs.dropout
        )
        self.dec_embedding = embed_cls(
        ...
        self.conv = nn.Sequential(
            Inception_Block_V1(d_model_, d_ff, num_kernels=num_kernels),
            nn.GELU(),
            Inception_Block_V1(d_ff, d_model_, num_kernels=num_kernels)
        )
    ...
    def period_extraction(self, x_enc):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if use_decomposition:
            ...
            res = self.period_extraction(seasonal.permute(0, 2, 1))

            if use_channel_independent:
                for i in range(self.mlp_layers):
                    t1 = self.mlp_time_list[i](trend)
                    t2 = self.mlp_feat_list[i](trend.permute(0, 2, 1)).permute(0, 2, 1)
                    if i < self.mlp_layers - 1:
                        trend = self.lin_fuse_list[i](t1 + t2) + x_enc.permute(0, 2, 1)
                    else:
                        trend = self.lin_fuse_list[i](t1 + t2)
