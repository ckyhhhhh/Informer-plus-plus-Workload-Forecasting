...
def get_q_k(input_size, window_size, stride, device):
    ...
    # Find corresponding k for each q by intra-scale, lower-scale, upper-scale order
    # Layer 1
    for i in range(input_size):
        mask[i, 0:window_size] = i + torch.arange(window_size) - window_size // 2
        # Set to -1 when window exceeds sequence right boundary
    ...
    for i in range(second_length):
        ...
        # Set to -1 when window exceeds sequence left boundary
...
class GraphSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # attn_weights.size(): (batch_size, L, num_heads, 11) Note: is_t1_diagonaled=False for q and k attention
        ...
        # Multiply attention scores with V, note: is_t1_diagonaled=True
